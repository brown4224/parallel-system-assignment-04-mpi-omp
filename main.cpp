...
void writeFile(int* new_data, void *ptr) {
    data *tdata = (data *) ptr;
    ...
    try
    {
        ...
        if(fileAppend.is_open()){
            ...
            fileAppend.write((char*) new_data, sizeof(data));

            fileAppend.close();

        } else {
            ofstream fileOut (tdata->local_file, ios::binary);
            fileOut.write((char*) new_data, sizeof(data));
            fileOut.close();
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < tdata.num_iterations; i++) {
        ...
        MPI_Scatter(tdata.readBuffer, *tdata.messageSize, MPI_INT, tdata.local_buffer, *tdata.messageSize, MPI_INT,
                    tdata.root, MPI_COMM_WORLD);
